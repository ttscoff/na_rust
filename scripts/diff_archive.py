...
def run_archive(bin_path: Path, args: List[str], cwd: Path, fixture_name: str) -> RunResult:
    try:
        proc = subprocess.run(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Diff Ruby and Rust `na archive` behavior.")
    parser.add_argument(
        "--ruby-na",
    ...
    parser.add_argument(
        "--fixtures",
        default="fixtures/archive",
        help="Directory containing archive fixtures (default: fixtures/archive)",
    )
    parser.add_argument(
        "--scenarios",
        default="fixtures/archive/scenarios.json",
        help="Scenario JSON file (default: fixtures/archive/scenarios.json)",
    )
    ...
    failures = 0
    skipped = 0
    print(f"Running {len(scenarios)} archive scenario(s)")
    for scenario in scenarios:
        ...
        with tempfile.TemporaryDirectory(prefix="na_rust_diff_archive_") as ruby_tmp, tempfile.TemporaryDirectory(
            prefix="na_rust_diff_archive_"
        ) as rust_tmp:
            ...
            ruby_result = run_archive(ruby_na, scenario.args, ruby_dir, fixture_name)
            rust_result = run_archive(rust_na, scenario.args, rust_dir, fixture_name)
