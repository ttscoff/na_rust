...
def main() -> int:
    parser = argparse.ArgumentParser(description="Diff Ruby and Rust `na find` outputs.")
    parser.add_argument(
        "--ruby-na",
    ...
    parser.add_argument(
        "--fixtures",
        default="fixtures/find",
        help="Directory containing taskpaper fixture files (default: fixtures/find)",
    )
    parser.add_argument(
        "--scenarios",
        default="fixtures/find/scenarios.json",
        help="Scenario JSON file (default: fixtures/find/scenarios.json)",
    )
    ...
    failures = 0
    print(f"Running {len(scenarios)} find scenario(s)")
    for scenario in scenarios:
        ...
        with tempfile.TemporaryDirectory(prefix="na_rust_diff_find_") as tmp:
            tmp_path = Path(tmp)
